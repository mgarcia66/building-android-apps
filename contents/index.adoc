...
* Android Studio, version 3.0 or higher. You can download Android Studio, along with the latest Android SDK, from https://developer.android.com/studio/index.html . The system requirements for the IDE are also found at that link.
...
====
include::sample[dir="groovy-dsl/code/app",files="build.gradle[tags=apply-plugin]"]
include::sample[dir="kotlin-dsl/code/app",files="build.gradle.kts[tags=apply-plugin]"]
====
...
====
include::sample[dir="groovy-dsl/code/app",files="build.gradle[tags=configure-plugin]"]
include::sample[dir="kotlin-dsl/code/app",files="build.gradle.kts[tags=configure-plugin]"]
====
...
====
include::sample[dir="groovy-dsl/code/app",files="build.gradle[tags=dependencies]"]
include::sample[dir="kotlin-dsl/code/app",files="build.gradle.kts[tags=dependencies]"]
====
...
This will run many tasks and eventually return "Build Successful". To see the resulting APK (Android package, the deployable version of an Android app), look in the directory `app/build/outputs/apk`. There you will find a `debug` and a `release` directories.
...
You'll see that there is an `installDebug` task and an `uninstallDebug` task, an `uninstallRelease` task, and even an `uninstallAll` task. Conspicuous by its absence, however, is an `installRelease` task. That task is only available if you create a signing configuration for a release key, which Gradle can use to create a signed release APK.
